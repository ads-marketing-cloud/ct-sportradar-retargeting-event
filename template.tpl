___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Sportradar Retargeting Template",
  "categories": ["TAG_MANAGEMENT", "PERSONALIZATION"],
  "brand": {
    "id": "sportradar_ads",
    "displayName": "Sportradar Ad:s",
    "thumbnail": "data:image/png;base64,"
  },
  "description": "Sportradar Retargeting Template",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "matchId",
    "displayName": "matchId",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "srMatchId",
    "displayName": "srMatchId",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "teamHomeName",
    "displayName": "teamHomeName",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "teamHomeNameShort",
    "displayName": "teamHomeNameShort",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "teamHomeLogo",
    "displayName": "teamHomeLogo",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "teamAwayName",
    "displayName": "teamAwayName",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "teamAwayNameShort",
    "displayName": "teamAwayNameShort",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "teamAwayLogo",
    "displayName": "teamAwayLogo",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "scheduled",
    "displayName": "scheduled",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "status",
    "displayName": "status",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "tournamentName",
    "displayName": "tournamentName",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "sport",
    "displayName": "sport",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "oddsHome",
    "displayName": "oddsHome",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "oddsDraw",
    "displayName": "oddsDraw",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "oddsAway",
    "displayName": "oddsAway",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "language",
    "displayName": "language",
    "simpleValueType": true
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const callInWindow = require('callInWindow');
const copyFromWindow = require('copyFromWindow');

if (!copyFromWindow("srtmCommands")) {
  data.gtmOnFailure();
  return;
}

callInWindow('srtmCommands.push', {
  event: "srt.retargeting",
  payload: {
    sportsEvent: {
      match_id: data.matchId,
      sr_match_id: data.srMatchId,
      team_home_name: data.teamHomeName,
      team_home_name_short: data.teamHomeNameShort,
      team_home_logo: data.teamHomeLogo,
      team_away_name: data.teamAwayName,
      team_away_name_short: data.teamAwayNameShort,
      team_away_logo: data.teamAwayLogo,
      scheduled: data.scheduled,
      status: data.status,
      tournament_name: data.tournamentName,
      sport: data.sport,
      odds_home: data.oddsHome,
      odds_draw: data.oddsDraw,
      odds_away: data.oddsAway,
      language: data.language,
    },
  },
});

data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "srtmCommands.push"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "srtmCommands"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios:
- name: Run event with missing srtmCommands
  code: |-
    runCode();

    // Verify that the tag finished successfully.
    assertApi('gtmOnFailure').wasCalled();
    assertApi('gtmOnSuccess').wasNotCalled();
- name: Run event with found srtmCommands
  code: |-
    const setInWindow = require('setInWindow');
    setInWindow("srtmCommands", []);

    runCode();

    assertApi('gtmOnFailure').wasNotCalled();
    assertApi('gtmOnSuccess').wasCalled();
- name: should push event with correct values
  code: |-
    const setInWindow = require('setInWindow');
    const mockData = {
      matchId: '1',
      srMatchId: '2',
      teamHomeName: 'My home team',
      teamHomeNameShort: 'MHT',
      teamHomeLogo: 'home logo',
      teamAwayName: 'My away team',
      teamAwayNameShort: 'MAT',
      teamAwayLogo: 'away logo',
      scheduled: '2019-09-07T15:50+00Z',
      status: 'CLOSED',
      tournamentName: 'My tournament',
      sport: 'soccer',
      oddsHome: '2',
      oddsDraw: '5',
      oddsAway: '10',
      language: 'en',
    };
    setInWindow('srtmCommands', [], true);

    runCode(mockData);

    const copyFromWindow = require('copyFromWindow');
    const srtmCommands = copyFromWindow('srtmCommands');
    assertThat(srtmCommands).isEqualTo([{
      event: "srt.retargeting",
      payload: {
        sportsEvent: {
          match_id: mockData.matchId,
          sr_match_id: mockData.srMatchId,
          team_home_name: mockData.teamHomeName,
          team_home_name_short: mockData.teamHomeNameShort,
          team_home_logo: mockData.teamHomeLogo,
          team_away_name: mockData.teamAwayName,
          team_away_name_short: mockData.teamAwayNameShort,
          team_away_logo: mockData.teamAwayLogo,
          scheduled: mockData.scheduled,
          status: mockData.status,
          tournament_name: mockData.tournamentName,
          sport: mockData.sport,
          odds_home: mockData.oddsHome,
          odds_draw: mockData.oddsDraw,
          odds_away: mockData.oddsAway,
          language: mockData.language,
        },
      },
    }]);
setup: ''


___NOTES___

Created on 3/3/2023, 11:40:46 AM


